...
void core1_SPI(){
  ...
  while(1){
    ...
    adcRead(1,adcP,(uint8_t*)(&ADCBuff[(potCounter & 0x07) + 8]) ) ;
...
void loop() {
  ...
  // process data
  for (int i = 0; i < 8; i++){
    buff1[i] = time_us_32() * ADCBuff[i+8] ; //Sawtooth wave based on potentiometer reading, can be used for lookup table
  }
